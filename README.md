...
Этот форк замечательного андроид приложения [wireguard-turn-android](https://github.com/kiper292/wireguard-turn-android).
В нем проведена работа по очень hot редизайну, а также переработан сценарий ввода ссылки, на которой поднимается TURN-туннель.
В данном форке реализуется автоматическая генерация ссылки на TURN-звонок либо простая передача уже готовой ссылки через единое поле ввода, независимо от выбранного туннеля.
Теперь независимо от конфига можно указать ссылку либо токен доступа вида `$...` в отдельном поле ввода на экране выбора узла соединения.
Токен доступа вида `$...` можно получить в консоли разработчика в логах запросов на известном сайте.

Далее текст редми из вышестоящего форка.
